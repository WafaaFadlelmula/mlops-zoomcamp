...
# Updated connection strings for homework settings
CONNECTION_STRING = "host=db port=5432 user=postgres password=test"
CONNECTION_STRING_DB = CONNECTION_STRING + " dbname=hw"

@task
def prep_db():
	with psycopg.connect(CONNECTION_STRING, autocommit=True) as conn:
		res = conn.execute("SELECT 1 FROM pg_database WHERE datname='hw'")
		if len(res.fetchall()) == 0:
			conn.execute("create database hw;")
...
def batch_monitoring_backfill():
	...
	for i in range(0, 31):  # March has 31 days
